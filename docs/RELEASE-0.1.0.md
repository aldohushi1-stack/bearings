...
Everything below is Aldo's to run (or Claude via the GitHub web upload page in Chrome, as with Glassbox).
...
1. On npmjs.com (account **aldoh**, 2FA with the security key): Packages → *get-bearings* does not exist yet — trusted publishing can only be configured **after** the first publish, so the first release goes up by hand:
   ...
   (The name `get-bearings` was free on 22 Sep 2026; `bearings` is taken.)
...
4. Verify: `npx get-bearings@0.1.0 --version` prints `0.1.0`; `npx get-bearings init` in any folder installs the hook.
